...
export class MenuContainerComponent implements OnInit, OnDestroy {
  ...
  @Input()
  startAngles: Object; // the first wing starting angle

  // Emit the wing that is been selected
  @Output()
  wingSelected = new EventEmitter<IMenuWing>();
  ...
  @Output()
  menuBtnClicked = new EventEmitter<boolean>();

  // Emit true if the whole menu list is being spun
  @Output()
  menuListSpinning = new EventEmitter<boolean>();

  menuBtnStyle: Object;
  menuWingsStyle: Object;
  svgPath: string;
  wingsState: boolean; // A flag to indicate if the wings are open
  positionClass: string; // menu's position
  menuConfig: any;
  top: number; // for draggingMenu animation
  left: number; // for draggingMenu animation
  ...
  private isSpinning = false; // A flag to indicate if the menuList is spinning
  ...
  draggingState = {
    value: false,
    params: { top: this.top, left: this.left }
  };
  ...
  ngOnInit() {
    ...
    this.menuSpunSubscriptionId = this.spinService.wingSpun.subscribe(
      (data: number) => this.spinMenu(data)
    );
  }
  ...
  clickWing(wing: IMenuWing): void {
    this.wingSelected.emit(wing);
  }

  /**
   * Emit the wing that has been mouse over
   * */
  hoverWing(wing: IMenuWing): void {
    if (!this.isDragging && !this.isSpinning) {
      this.wingHovered.emit(wing);
    }
  }
  ...
  toggleMenu() {
    if (this.allowTransition) {
      ...
      this.menuBtnClicked.emit(this.wingsState);
    }
  }
   ...
   * */
  spinMenu(deg: number): void {
    this.renderer.setStyle(
      this.menuWingsElm.nativeElement,
      "transform",
      "rotate(" + deg + "deg)"
    );
  }
   ...
   * */
  toggleSpinningState(state: boolean): void {
    if (state) {
      this.menuListSpinning.emit(true);
    }
