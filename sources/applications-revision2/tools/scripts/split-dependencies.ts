...
const WEB_APPLICATION_PATH = path.join(
  process.cwd(),
  "apps",
  "web-application"
);
...
const splitPackagesInclude = (list: any) =>
  _.differenceWith(
    (string: any, fragment: any) => _.startsWith(fragment, string),
    list,
    EXTERNAL_DEPENDENCIES
  );

const getDependenciesList = _.flow([
  JSON.parse,
  _.at(["dependencies"]),
  _.first,
  _.keys
...
const externalDependencies = _.difference(
  dependenciesList,
  internalDependencies
);
const extendedExternalDependencies = _.concat(
  externalDependencies,
  FORCE_EXCLUDE
);

console.log(
  `Internal server dependencies: ${JSON.stringify(internalDependencies)}\n`
);
console.log(
  `External server dependencies: ${JSON.stringify(
    extendedExternalDependencies
  )}`
);
...
shell.exec(
  `yarn add --ignore-engines ${_.join(" ", extendedExternalDependencies)}`
);
