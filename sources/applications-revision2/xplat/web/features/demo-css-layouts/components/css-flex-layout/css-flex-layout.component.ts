...
import { CssFlexLayoutBaseComponent } from "@applications/features";

@Component({
  selector: "app-css-flex-layout",
  templateUrl: "css-flex-layout.component.html",
  styleUrls: ["css-flex-layout.component.scss"]
})
export class CssFlexLayoutComponent extends CssFlexLayoutBaseComponent
  implements OnInit {
  // Container
  ...
  ngOnInit() {
    ...
    this.flexForm.get("basis").valueChanges.subscribe(newValue => {
    ...
    this.flexForm.get("alignContent").valueChanges.subscribe(newValue => {
      this.itemsFormArray.controls.forEach((control, index) =>
        this.itemsFormArray
          .at(index)
          .get("alignSelf")
  ...
  getFlexOptions(index: number) {
    ...
    if (
      itemFlexOptions.grow ||
      itemFlexOptions.shrink ||
      itemFlexOptions.basis !== this.basis
    ) {
      return `${itemFlexOptions.grow} ${itemFlexOptions.shrink} ${
        itemFlexOptions.basis
      }`;
    } else {
  ...
  add(type: string) {
    ...
    if (type === "items") {
      this.itemsFormArray.push(
        this.formBuilder.group(this.itemFlexOptions(true))
      );
    } else {
      this.itemsFormArray.controls.forEach((control, index) =>
        this.itemsFormArray
