...
export class DemoCovalentUiUsersFormComponent extends DemoCovalentUiUsersFormBaseComponent implements OnInit {
  displayName: string;
  email: string;
  id: string;
  admin: boolean;
  ...
  async load(): Promise<void> {
    try {
      ...
      let user: CovalentUser = await this._userService.get(this.id).toPromise();
