...
export class UiOverlayService {
  ...
  private attachDialogContainer(overlayRef: OverlayRef, config: UiOverlayConfig, dialogRef: UiOverlayRef) {
    const injector = this.createInjector(config, dialogRef);

    const containerPortal = new ComponentPortal(UiOverlayComponent, null, injector);
    const containerRef: ComponentRef<UiOverlayComponent> = overlayRef.attach(containerPortal);

    return containerRef.instance;
  }

  private createInjector(config: UiOverlayConfig, dialogRef: UiOverlayRef): PortalInjector {
